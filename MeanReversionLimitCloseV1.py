...
class MomentumIndicatorNodeConfig:
    __slots__ = ('half_life', 'multiplier', 'exit_multiplier')
...
class MomentumIndicatorNode(Node[MomentumIndicatorNodeConfig, MomentumIndicatorResult]):
    ...
    def handle_ema_result(self, ema_result: ExponentialMovingAverageResult) -> None:
        ...
        upper1 = ema_result.sma_mean + self.config.exit_multiplier * math.sqrt(ema_result.sma_variance)
        lower1 = ema_result.sma_mean - self.config.exit_multiplier * math.sqrt(ema_result.sma_variance)
...
class MomentumSignalNodeConfig:
    __slots__ = ('half_life', 'threshold', 'MOMENTUM_SIGNAL_SIGNIFICANCE_THRESHOLD', 'VARIANCE_THERESHOLD',
                 'multiplier', 'exit_multiplier', 'symbol0', 'ordType0', 'StopLoss', 'orderQty0', 'bitmex_api_key', 'bitmex_api_secret',
                 'is_live')
...
class MomentumSignalNode(Node[MomentumSignalNodeConfig, Signal]):
    ...
    def __init__(self, engine: 'Engine', config: MomentumSignalNodeConfig) -> None:
        ...
        momentum_indicator_node_config = MomentumIndicatorNodeConfig(half_life=config.half_life, multiplier=config.multiplier, exit_multiplier=config.exit_multiplier)
    ...
    def handle_momenta(self, momenta: tuple[MomentumIndicatorResult, MomentumIndicatorResult, MomentumIndicatorResult]):
        ...
        partial_message_signal = f'[Current (MPP, upper, lower, significance, varRatio) is ({current_momentum.midpoint_price}, {current_momentum.upper}, {current_momentum.lower}, {current_momentum.significance}, {varRatio}).]'
        ...
        if current_momentum.midpoint_price < current_momentum.lower:
            ...
            if (current_momentum.significance > self.config.MOMENTUM_SIGNAL_SIGNIFICANCE_THRESHOLD and varRatio < self.config.VARIANCE_THERESHOLD and noBuy == 0):
                ...
                if positions['currentQty'] == 0:
                    ...
                    self.place_order(StopLoss[0]['orderID'], NewOrder[0]['orderID'])
                    logger.info(f'{partial_message2}:BUY order!')
            elif positions['currentQty'] < 0:
            ...
            noBuy, noSell = tradeHistory()
            if (current_momentum.significance > self.config.MOMENTUM_SIGNAL_SIGNIFICANCE_THRESHOLD and varRatio < self.config.VARIANCE_THERESHOLD and noSell == 0):
                logger.info(f'{partial_message_signal}: SELL signal!')
            ...
            logger.info(f'{partial_message_signal}: midpoint is between lower1 and upper1!' +
            ...
            if is_upper is not None:
                # cancel buy
                if is_upper:
                    logger.info(f'{partial_message_signal}: The midpoint price was reduced from above upper1 to below upper1.Then cancel SELL orders.')
                    self.cancel_by_symbol(self.config.symbol0, orderQty=Signal.SELL * self.config.orderQty0, ordType=self.config.ordType0)
                # cancel sell
                else:
                    logger.info(f'{partial_message_signal}: The midpoint price went up from below lower1 to above lower1.Then cancel BUY orders.')
                    self.cancel_by_symbol(self.config.symbol0, orderQty=Signal.BUY * self.config.orderQty0, ordType=self.config.ordType0)
            else:
                logger.info(f'{partial_message_signal}: Historical midpoint price are all between lower1 and upper1. There\'s no need to cancel orders.')

        else:
            logger.info(f'{partial_message_signal}: No signal (price within threshold).')
            return


    def tradeHistory():
        tradeHist = client.Execution.Execution_getTradeHistory(symbol=symbol).result()
